...
def compress():
    try:
        ...
        if os.path.exists(file_path):
            ...
            download_path = os.path.join(app.config['DOWNLOAD_FOLDER'], secure_filename(filename + '_compressed' + ext))
        ...
        return render_template('index.html', error_compress=str(e))



def compress_image_or_video(file_path, output_path):
    try:
        if file_path.lower().endswith(('.jpeg', '.png')):
        ...
        raise RuntimeError(f"Error compressing image or video: {e}")
...
def compress_text(file_path, output_path):
    try:
        ...
        raise RuntimeError(f"Error compressing text: {e}")
...
def decompress():
    try:
        ...
        if os.path.exists(file_path):
            filename, ext = os.path.splitext(os.path.basename(request.form['file_path']))
            download_path = os.path.join(app.config['DOWNLOAD_FOLDER'], secure_filename(filename + '_decompressed' + ext))
            decompressed_path = os.path.join(app.config['DOWNLOAD_FOLDER'], secure_filename(filename))
            if ext.lower() == '.compressed':
                decompress_lossless(file_path, decompressed_path)
            elif ext.lower() in ['.jpg', '.jpeg', '.png', '.gif', '.mp3', '.wav', '.ogg', '.mp4', '.avi', '.mov', '.txt', '.csv', '.doc', '.docx', '.xls', '.xlsx']:
            ...
            session['decompression_data'] = {
                ...
                'compressed_size': compressed_size,
                'compression_ratio': compression_ratio,
                'file_type': file_type  # Ajout du type de fichier dans les données de décompression
            }
        ...
        return render_template('index.html', error_decompress=str(e))
...
def decompress_lossy(compressed_file_path, output_path):
    try:
        ...
        elif file_extension in ['.txt', '.csv', '.doc', '.docx', '.xls', '.xlsx']:
        ...
        raise RuntimeError(f"Error during lossy decompression: {e}")
